    ...
    <>
      ...
      <article className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl overflow-hidden transition-all hover:border-[#3B82F6]/30">
        ...
        <div className="p-4 md:p-5 border-b border-[var(--border-color)]">
          <div className="flex items-center gap-3">
            <Link href={`/user/${post.author.username || post.author.id}`}>
              <div className="w-11 h-11 md:w-12 md:h-12 rounded-full overflow-hidden border-2 border-[var(--border-color)] hover:border-[#3B579A] transition-colors">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 flex-wrap">
                <Link href={`/user/${post.author.username || post.author.id}`} className="font-bold text-[var(--text-main)] hover:text-[#3B82F6] transition-colors text-sm md:text-base">
                  {post.author.name || 'Anonymous'}
                </Link>
                {post.author.isVerified && <VerifiedBadge size="sm" />}
              </div>
              <p className="text-xs text-[var(--text-muted)]">@{post.author.username || 'user'} • {formatDate(post.createdAt)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 md:p-5">
          <h2 className="text-lg md:text-xl font-bold text-[var(--text-main)] mb-2">{post.title}</h2>
          
          {/* === BAGIAN INI YANG DIUBAH === */}
          {/* Menggunakan dangerouslySetInnerHTML agar gambar muncul */}
          {post.content && (
            <div className={`text-sm md:text-base text-[var(--text-muted)] leading-relaxed ${!showFull ? 'line-clamp-3' : ''}`}>
          ...
          <div className="border-t border-[var(--border-color)] bg-[var(--bg-main)]/50 p-4 md:p-5 space-y-5 animate-in slide-in-from-top-2 duration-300">
            <div className="border-t border-[var(--border-color)] pt-4 mt-4">
              ...
              <form onSubmit={handleComment} className="flex gap-2 mt-4">
                <input type="text" placeholder="Tulis komentar..." value={commentText} onChange={(e) => setCommentText(e.target.value)} className="flex-1 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-full px-4 py-2 text-xs text-[var(--text-main)] outline-none focus:border-[#3B82F6] transition-colors" />
                <button type="submit" disabled={!commentText.trim() || loading} className="px-4 py-2 bg-[#3B82F6] text-white text-xs font-bold rounded-full hover:bg-[#2563EB] disabled:opacity-50 transition-all">Kirim</button>
              </form>
            </div>
          </div>
        )}
        ...
        <div className="px-4 md:px-5 py-3 border-t border-[var(--border-color)] flex items-center gap-4">
          <button onClick={handleLike} disabled={loading || !currentUserId} className={`flex items-center gap-2 text-sm transition-all ${isLiked ? 'text-red-500' : 'text-[var(--text-muted)] hover:text-red-500'}`}>
          ...
          <button onClick={() => setShowFull(!showFull)} className="ml-auto text-xs font-bold text-[#3B82F6] hover:underline flex items-center gap-1">
            {showFull ? (<>Tutup <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="18 15 12 9 6 15"></polyline></svg></>) : (<>Baca <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="6 9 12 15 18 9"></polyline></svg></>)}
          </button>
        </div>
      </article>
    </>
