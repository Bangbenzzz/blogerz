  ...
  return (
    <>
      ...
      <article className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl overflow-hidden transition-all hover:border-[#3B82F6]/30 relative group/card">
        ...
        {/* Hanya muncul jika user adalah pemilik post */}
        {isOwner && (
          <Link 
            href={`/edit/${post.id}`}
            className="absolute top-3 right-3 z-10 w-9 h-9 rounded-full bg-[var(--bg-main)] border border-[var(--border-color)] flex items-center justify-center text-[var(--text-muted)] hover:text-[#3B82F6] hover:border-[#3B82F6] transition-all opacity-0 group-hover/card:opacity-100 shadow-lg"
            title="Edit Post"
          >
          ...
          <div className="border-t border-[var(--border-color)] bg-[var(--bg-main)]/50 p-4 md:p-5 space-y-5 animate-in slide-in-from-top-2 duration-300">
            ...
            <div className="border-t border-[var(--border-color)] pt-4 mt-4">
              ...
              {comments.length > 0 ? (
                ...
                <p className="text-xs text-[var(--text-muted)] italic text-center py-4">Belum ada komentar. Jadilah yang pertama!</p>
              )}
        ...
        <div className="px-4 md:px-5 py-3 border-t border-[var(--border-color)] flex items-center gap-4">
          ...
          >
            {showFull ? (
              <>Tutup <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="18 15 12 9 6 15"></polyline></svg></>
            ) : (
              <>Baca <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="6 9 12 15 18 9"></polyline></svg></>
            )}
          </button>
        </div>
      </article>
