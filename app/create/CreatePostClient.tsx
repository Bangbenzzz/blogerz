...
const ImageComponent = ({ node, updateAttributes, deleteNode }: any) => {
  ...
  const width = node.attrs.width || '30%'
  ...
  const containerStyle: React.CSSProperties = isFloat ? {
    float: alignment === 'floatLeft' ? 'left' : 'right',
    width: width,
    margin: alignment === 'floatLeft' ? '0 1rem 0.5rem 0' : '0 0 0.5rem 1rem',
    clear: 'both',
  } : {
    display: 'flex',
  ...
  return (
    <NodeViewWrapper className="image-view-wrapper" style={containerStyle}>
      <div 
        className={`relative cursor-pointer transition-all inline-block ${selected ? 'ring-2 ring-orange-500' : 'hover:ring-2 hover:ring-gray-500'}`}
        style={{ width: isFloat ? '100%' : width, maxWidth: '100%' }}
      ...
      >
        ...
        {selected && (
          <div 
            className="absolute -top-12 left-1/2 transform -translate-x-1/2 bg-[#3c4043] shadow-xl rounded-lg border border-gray-600 p-1 flex gap-1 z-50 image-toolbar"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Image Alignment Buttons */}
            <button onClick={() => updateAttributes({ textAlign: 'left' })} className="p-1.5 hover:bg-gray-600 rounded text-white" title="Kiri"><svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="17" y1="10" x2="3" y2="10"/><line x1="21" y1="6" x2="3" y2="6"/><line x1="21" y1="14" x2="3" y2="14"/><line x1="17" y1="18" x2="3" y2="18"/></svg></button>
            <button onClick={() => updateAttributes({ textAlign: 'center' })} className="p-1.5 hover:bg-gray-600 rounded text-white" title="Tengah"><svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="10" x2="6" y2="10"/><line x1="21" y1="6" x2="3" y2="6"/><line x1="21" y1="14" x2="3" y2="14"/><line x1="18" y1="18" x2="6" y2="18"/></svg></button>
            <button onClick={() => updateAttributes({ textAlign: 'right' })} className="p-1.5 hover:bg-gray-600 rounded text-white" title="Kanan"><svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="21" y1="10" x2="7" y2="10"/><line x1="21" y1="6" x2="3" y2="6"/><line x1="21" y1="14" x2="3" y2="14"/><line x1="21" y1="18" x2="7" y2="18"/></svg></button>
            
            <div className="w-px h-6 bg-gray-500 mx-1 self-center"></div>

            {/* Image Size Buttons */}
            <button onClick={() => updateAttributes({ width: '25%' })} className="text-xs px-2 hover:bg-gray-600 rounded text-white font-medium">S</button>
            <button onClick={() => updateAttributes({ width: '50%' })} className="text-xs px-2 hover:bg-gray-600 rounded text-white font-medium">M</button>
            <button onClick={() => updateAttributes({ width: '100%' })} className="text-xs px-2 hover:bg-gray-600 rounded text-white font-medium">XL</button>
            
            <div className="w-px h-6 bg-gray-500 mx-1 self-center"></div>
            
            <button onClick={deleteNode} className="p-1.5 hover:bg-red-900/50 rounded text-red-400" title="Hapus">
...
const CustomImage = Image.extend({
  addAttributes() {
    return {
      ...this.parent?.(),
      width: { default: '30%', renderHTML: attrs => ({ style: `width: ${attrs.width}; max-width: 100%;` }) },
...
export default function CreatePostClient({ userId, profile }: { userId: string, profile: any }) {
  ...
  const TBtn = ({ onClick, active, disabled, children, title }: any) => (
    <button 
      ...
      disabled={disabled} 
      title={title} 
      className={`p-1.5 sm:p-2 rounded hover:bg-[#3c4043] transition-colors disabled:opacity-30 flex items-center justify-center ${active ? 'bg-[#3c4043] text-orange-400' : 'text-gray-300'}`}
    >
      {children}
    </button>
  )
  ...
  return (
    <div className="min-h-screen bg-[#202124] flex flex-col font-sans" onClick={closeAllDropdowns}>
      ...
      <header className="sticky top-0 z-[200] bg-[#292a2d] border-b border-gray-700 shadow-sm">
        <div className="flex items-center justify-between px-3 sm:px-4 py-2 sm:py-3">
          ...
          <button onClick={handleSubmit} disabled={loading || !title.trim()} className="px-4 sm:px-5 py-1.5 sm:py-2 bg-orange-500 hover:bg-orange-600 text-white text-sm font-medium rounded-full disabled:opacity-50 transition-colors flex items-center gap-2">
             ...
             <span>{loading ? 'Menyimpan...' : 'Publikasikan'}</span>
          </button>
        </div>
      </header>

      {/* TOOLBAR */}
      <div className="sticky top-[53px] sm:top-[58px] z-[150] bg-[#292a2d] border-b border-gray-700 px-1 sm:px-4 py-2">
        <div className="flex flex-wrap items-center gap-1 relative">
          ...
          <div className="relative">
            <button onClick={(e) => { e.stopPropagation(); closeAllDropdowns(); setShowFontMenu(!showFontMenu) }} className="flex items-center gap-1 px-2 py-1.5 hover:bg-[#3c4043] rounded text-xs text-gray-300 border border-transparent hover:border-gray-600">
            ...
            {showFontMenu && (
              <div className="absolute top-full left-0 mt-1 bg-[#292a2d] border border-gray-600 rounded-lg shadow-xl py-1 min-w-[150px] max-h-60 overflow-y-auto z-[300]">
                {fontList.map(font => (
                  <button key={font} onClick={() => { editor.chain().focus().setFontFamily(font).run(); setShowFontMenu(false) }} style={{ fontFamily: font }} className="w-full px-4 py-2 text-left hover:bg-[#3c4043] text-sm text-gray-300">{font}</button>
                ))}
              </div>
            )}
          </div>
          ...
          {/* HEADING DROPDOWN */}
          <div className="relative">
            <button onClick={(e) => { e.stopPropagation(); closeAllDropdowns(); setShowHeadingMenu(!showHeadingMenu) }} className="flex items-center gap-1 px-2 py-1.5 hover:bg-[#3c4043] rounded text-xs text-gray-300 border border-transparent hover:border-gray-600">
              <span>{editor.isActive('heading', { level: 1 }) ? 'Judul Besar' : editor.isActive('heading', { level: 2 }) ? 'Judul Sedang' : 'Normal'}</span>
              <svg className="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 9 12 15 18 9"/></svg>
            </button>
            {showHeadingMenu && (
              <div className="absolute top-full left-0 mt-1 bg-[#292a2d] border border-gray-600 rounded-lg shadow-xl py-1 min-w-[140px] z-[300]">
                <button onClick={() => { editor.chain().focus().setParagraph().run(); setShowHeadingMenu(false) }} className="w-full px-4 py-2 text-left hover:bg-[#3c4043] text-sm text-gray-300">Normal</button>
                <button onClick={() => { editor.chain().focus().toggleHeading({ level: 1 }).run(); setShowHeadingMenu(false) }} className="w-full px-4 py-2 text-left hover:bg-[#3c4043] text-lg font-bold text-white">Judul Besar</button>
                <button onClick={() => { editor.chain().focus().toggleHeading({ level: 2 }).run(); setShowHeadingMenu(false) }} className="w-full px-4 py-2 text-left hover:bg-[#3c4043] text-base font-bold text-white">Judul Sedang</button>
                <button onClick={() => { editor.chain().focus().toggleHeading({ level: 3 }).run(); setShowHeadingMenu(false) }} className="w-full px-4 py-2 text-left hover:bg-[#3c4043] text-sm font-bold text-white">Judul Kecil</button>
              </div>
            )}
          </div>
          ...
          {/* COLOR PICKER */}
          <div className="relative">
            <button onClick={(e) => { e.stopPropagation(); closeAllDropdowns(); setShowColorPicker(!showColorPicker) }} className="p-1.5 rounded hover:bg-[#3c4043] text-gray-300" title="Warna Teks">
            ...
            {showColorPicker && ( <div className="absolute top-full left-0 mt-1 bg-[#292a2d] border border-gray-600 rounded-lg shadow-xl p-2 grid grid-cols-5 gap-1 z-[300] w-40">{['#FFFFFF', '#E53935', '#FB8C00', '#43A047', '#1E88E5', '#8E24AA', '#6D4C41', '#757575', '#546E7A', '#EC407A'].map(c => (<button key={c} onClick={() => { editor.chain().focus().setColor(c).run(); setShowColorPicker(false) }} className="w-6 h-6 rounded-full border border-gray-500 hover:scale-110 transition-transform" style={{ backgroundColor: c }} title={c}></button>))}</div> )}
          </div>

          <div className="w-px h-5 bg-gray-600 mx-1"></div>

          {/* ========================================== */}
          {/* TEXT ALIGNMENT BUTTONS (NEW) */}
          {/* ========================================== */}
          <TBtn
          ...
          {/* EMOJI PICKER */}
          <div className="relative">
            <TBtn onClick={(e: any) => { e.stopPropagation(); closeAllDropdowns(); setShowEmojiPicker(!showEmojiPicker) }} title="Emoji" active={showEmojiPicker}>😊</TBtn>
            {showEmojiPicker && (
              <div 
                className="absolute top-full right-0 mt-2 p-2 bg-[#292a2d] border border-gray-600 rounded-lg shadow-xl z-[300] w-72" 
                onClick={(e) => e.stopPropagation()}
              >
      ...
      {/* EDITOR AREA */}
      <main className="flex-1 bg-[#202124] w-full overflow-y-auto">
      ...
      {/* FOOTER */}
      <footer className="bg-[#292a2d] border-t border-gray-700 px-4 py-2 text-xs text-gray-400 flex justify-between items-center">
      ...
      <style jsx global>{`
        ...
        /* Styling for Headings Alignment */
        ...
        .image-view-wrapper { display: block; overflow: hidden; }
      `}</style>
    </div>
  )
}